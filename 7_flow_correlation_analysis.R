...
cluster_freq.table <- cluster_freq.table[cluster_freq.table$ClusterID != "Tother", ]
ratio.df <- cluster_freq.table[ ,c(1:3)] %>% pivot_wider(names_from = "ClusterID", values_from = "Count") %>% mutate(trm_tinf_ratio = Trm/Tinf)
...
p <- ggplot(ratio.df, aes(y = trm_tinf_ratio, x = cellSource)) + 
    ...
    theme(panel.grid.major = element_line(color = "grey", size = 0.25),
          ...
          ) + scale_color_manual(labels = unique(seu.obj$name2), values = levels(seu.obj$colz))
ggsave(paste("./output/", outName, "/", outName, "_RatioBoxplot.png", sep = ""), width = 3, height = 4)
